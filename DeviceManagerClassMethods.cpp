...
void DeviceManager::showAllDevices() const
{
    ...
    for (int i = 0; i < devices.size(); i++)
    {
    	//Dynamically casts the found index to type DevicePowerInterface, which
    	//has access to the turn on/off functions for the derived classes
    	DevicePowerInterface* power = dynamic_cast<DevicePowerInterface*>(devices[i]);

    	//Dynamically casts the found index to type BatteryInterface, in this
    	//case it is not being used but is important for being able to access
    	//the battery percentage of devices with that feature.
    	BatteryInterface* batt = dynamic_cast<BatteryInterface*>(devices[i]);

        cout << "Device Name: " << devices[i]->getName() << endl;
        cout << "Power Status: " << (power->getPower() ? "On" : "Off") << endl;

/*******************************************************************************
 * This section is commented out because it improperly calls for a feature that
 * is not defined in all of the derived classes.
 ******************************************************************************/
//        cout << "Battery: " << batt->getBattery() << "%" << endl;
